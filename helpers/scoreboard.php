	        ...
	        if ($row['user'] == $_SESSION['username']) {
						...
						<tr>
						  	<tr " . $rowTags . ">
							    ...
							    <td><b>"; if ($_SESSION['admin'] == 'TRUE') {echo "<a href='userinfo.php?user=" . $row['user'] . "'>" . $row['user'] . $nameAddition . "</a>";} else {echo $row['user'] . $nameAddition;} echo "</b></td>
							    <td><b>" . $row["score"] . "</b></td>";
									if ($_SESSION['admin'] == 'TRUE') {
	        	...
	        	echo "
						<tr>
						  	<tr " . $rowTags . ">
						    <td style='width:40px; border-right:1px solid #ddd; text-align:center'><b>$rank</b></td>
						    <td>"; if ($_SESSION['admin'] == 'TRUE') {echo "<a href='userinfo.php?user=" . $row['user'] . "'>" . $row['user'] . $nameAddition . "</a>";} else {echo $row['user'] . $nameAddition;} echo "</td>
						    <td>" . $row["score"] . "</td>";
								if ($_SESSION['admin'] == 'TRUE') {
									$result2 = mysqli_query($conn, "SELECT lastSeen FROM users WHERE username = '" . $row['user'] . "'");
									$row2 = mysqli_fetch_array($result2);
